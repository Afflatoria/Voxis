...
2. Adjust **Speaking rate** / **Energy** (mapped to F5-TTS speed and loudness).
3. Click **Generate** — audio streams in chunks as each text batch completes.
...
| Control | M1 status |
|---------|-----------|
| `speaking_rate` | F5-TTS `speed` parameter |
| `energy` | F5-TTS `target_rms` loudness |
...
- **Chunk streaming, not token streaming** — F5-TTS synthesizes each text batch fully, then yields fixed-size waveform chunks.
